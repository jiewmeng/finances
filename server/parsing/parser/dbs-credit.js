  ...
  const renderPage = async (data) => {
    try {
      ...
      processed.forEach((item, idx) => {
        ...
        if (/^\d{2} \w{3}$/.test(item.text) && Math.abs(item.x - 64) < 2) {
          const txn = {
            date: null,
            description: '',
            amount: 0,
          ...
          while (true) {
            ...
            /^\d{2} \w{3}$/.test(processed[lookAheadIdx].text) && Math.abs(processed[lookAheadIdx].x - 64) < 2 ||
            ...
            processed[lookAheadIdx].text === 'SUB-TOTAL:') {
              ...
              if (processed[lookAheadIdx] && processed[lookAheadIdx].text === 'TOTAL:') {
                // Get the ending balance
                const endBalanceStr = processed[lookAheadIdx + 1].text.replace(',', '')
